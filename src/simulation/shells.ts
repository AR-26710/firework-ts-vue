...
import type { GlitterConfig, StarConfig } from './shell-types';
import { renderTextToPositions } from './text-renderer';
...
class Shell {
	...
	private burstText(x: number, y: number, color: string | undefined, glitterConfig: GlitterConfig) {
		const { positions } = renderTextToPositions(this.textString);
